...
def generate_readme_with_table(config: dict, output_dir: Path) -> None:
...
def main():
    ...
    if not args.skip_search:
        ...
        if not args.skip_researcher:
            try:
                ...
                print(f"[build] Researcher failed ({e}), falling back to arXiv API")
    ...
    generate_readme_with_table(config, output_dir)
