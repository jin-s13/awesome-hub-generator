...
ENV_FIELDS = [
    ...
    ("MINERU_BASE_URL", "MinerU Base URL", False, "https://mineru.net/api/v4"),
    ("MINERU_API_MODE", "MinerU API Mode", False, "auto"),
    # SMTP
...
def flatten_config_dict(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    flat["mineru_base_url"] = pp.get("base_url", "https://mineru.net/api/v4")
    flat["mineru_api_mode"] = pp.get("api_mode", "auto")
...
def _is_local_mineru_endpoint(base_url: str, api_mode: str) -> bool:
    api_mode = (api_mode or "auto").lower()
    if api_mode == "local":
        return True
    if api_mode == "official":
        return False
    return bool(base_url) and "mineru.net/api/v4" not in _normalize_mineru_base_url(base_url)
...
def validate_mineru_connection(
    api_key: str,
    ...
    """
    env_values = read_env()
    base_url = _normalize_mineru_base_url(
        base_url or env_values.get("MINERU_BASE_URL") or "https://mineru.net/api/v4"
    )
    api_mode = api_mode or env_values.get("MINERU_API_MODE") or "auto"

    if _is_local_mineru_endpoint(base_url, api_mode):
        try:
            with urlreq.urlopen(f"{base_url}/health", timeout=10) as resp:
                body = resp.read().decode("utf-8")
                data = json.loads(body)
            if data.get("status") == "healthy":
                version = data.get("version", "unknown")
                return True, f"✅ 本地 MinerU 服务连接正常（version={version}）。"
            return False, f"⚠️ 本地 MinerU 服务响应异常: {data}"
        except Exception as e:
            return False, f"⚠️ 无法连接本地 MinerU 服务: {e}"
    ...
    try:
        ...
        payload = json.dumps({"url": "", "model_version": "pipeline"}).encode("utf-8")
        req = urlreq.Request(
            url,
            data=payload,
            headers={
