...
def _json_mode_unsupported(error: Exception) -> bool:
    ...
    return "response_format" in text and "json" in text and (
        "not support" in text or "unsupported" in text or "invalid" in text
    )
...
class AnalysisAgent:
    ...
    def _create_json_completion(self, client, *, model: str, messages: List[Dict[str, str]], temperature: float):
        kwargs = {
            "model": model,
            "messages": messages,
        ...
        try:
            ...
            if _json_mode_unsupported(e):
                logger.warning("当前模型不支持 JSON response_format，退回普通 Chat Completions")
    ...
    def _call_cheap_llm(self, prompt: str) -> str:
        ...
        def _do_call():
            try:
                resp = self._create_json_completion(
                    self.cheap_client,
