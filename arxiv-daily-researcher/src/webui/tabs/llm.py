...
def render(env_values: dict, _config_values: dict):
    ...
    mineru_base_url = st.text_input(
        "MinerU Base URL",
        value=env_values.get("MINERU_BASE_URL", "https://mineru.net/api/v4"),
        key="mineru_base_url",
        help="官方默认 https://mineru.net/api/v4；本地服务例如 http://mineru.local:2511",
    )
    mineru_mode_options = ["auto", "official", "local"]
    current_mineru_mode = env_values.get("MINERU_API_MODE", "auto")
    if current_mineru_mode not in mineru_mode_options:
        current_mineru_mode = "auto"
    mineru_api_mode = st.selectbox(
        "MinerU API Mode",
        options=mineru_mode_options,
        index=mineru_mode_options.index(current_mineru_mode),
        key="mineru_api_mode",
        help="auto 会根据 Base URL 自动选择官方或本地协议。",
    )
    ...
    if test_mineru_btn:
        with st.spinner(t("testing_mineru")):
            ...
            ok, msg = validate_mineru_connection(mineru_key, mineru_base_url, mineru_api_mode)
        if ok:
            st.success(msg)
        else:
            st.warning(msg)
...
def collect(env_values: dict, _config_values: dict) -> dict:
    ...
    return {
        ...
        "MINERU_BASE_URL": st.session_state.get("mineru_base_url", ""),
        "MINERU_API_MODE": st.session_state.get("mineru_api_mode", "auto"),
    }
