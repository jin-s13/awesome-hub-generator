...
def should_force_official_mineru() -> bool:
...
class Settings(BaseSettings):
    ...
    MINERU_BASE_URL: str = DEFAULT_MINERU_BASE_URL  # MinerU API 地址；本地服务如 http://mineru.local:2511
    MINERU_API_MODE: str = "auto"  # auto / official / local
    ...
    def load_from_search_config(self, config_path: Optional[Path] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if "pdf_parser" in config:
                ...
                self.MINERU_BASE_URL = pdf_cfg.get("base_url", self.MINERU_BASE_URL)
                self.MINERU_API_MODE = pdf_cfg.get("api_mode", self.MINERU_API_MODE)
                if should_force_official_mineru():
                    self.MINERU_BASE_URL = DEFAULT_MINERU_BASE_URL
                    self.MINERU_API_MODE = "official"
